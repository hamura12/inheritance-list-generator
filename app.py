...
# ── 開発経過 ───────────────────────────────────────────────────
st.divider()
with st.expander('📖 開発経過・更新履歴'):
    st.markdown("""
本ツールは、より使いやすく安全にご利用いただけるよう、継続的に改善を重ねています。
これまでの主な開発の歩みをご紹介します。

##### 🏗 基本機能の構築
- 法定相続情報一覧図のExcelひな形を自動生成する仕組みを構築
- 相続関係（配偶者・子・親・兄弟姉妹）に応じて最適な書式を自動判定
- 兄弟姉妹が死亡し代襲相続が生じないケースにも対応

##### 🎨 デザイン・操作性の向上
- 法律書類にふさわしい、重厚感のある専門的なデザインへ刷新
- 入力内容に応じて相続関係図をリアルタイム表示するプレビュー機能を追加
- 選択ボタンやアニメーションなど、直感的に操作できるUIへ改良
- 入力をすべてクリアできるリセット機能を追加
- 法務局の公式案内ページへの参考リンクを設置

##### 👪 代襲相続への対応
- お子さまが亡くなっている場合に、お孫さま（代襲者）の人数を入力できる機能を追加
- 続柄を結ぶ関係線を自動で描画する仕組みを整備
- 当該書式は現在さらに精度を高めるため調整中です

##### 🔒 安全性・プライバシーの強化
- 氏名・住所などの個人情報はツールに入力せず、出力後に手入力いただく方針を明示
- 入力内容を保存・送信しない設計を維持
- 表示まわりの安全対策を強化し、より安心してご利用いただける構成に改善

> 今後も、利用者の皆さまの声を反映しながら改善を続けてまいります。
""")
