...
with st.expander('🔍 参考書式をどう解析したか（開発者向けヒント）'):
    st.markdown("""
さらに発展したツールを作りたい方へ、本ツールが**参考書式（Excel）をどのように
読み解いてプログラム化したか**、その手順をご紹介します。

##### ① 書式を「3つの構造」に分解して読み取る
参考のExcelファイルを `openpyxl` で開き、見た目を構成する要素を
数値データとして抽出します。

| 抽出する要素 | 内容 |
|---|---|
| **行の高さ** | 各行の高さ（被相続人欄は高く、補足欄は低い、など） |
| **セルの結合** | どのセル範囲が結合され、何の項目か（「最後の住所」等） |
...
##### ③ 「列のゾーン」に分けて配置ルールを見つける
横方向を3つの区画に分けて考えると、規則性が見えてきます。
...
##### ④ 人数に応じて座標を動かす
「1人増えるごとに6行ずつ下にずらす」といった**規則**を見つけ、
固定の書式ではなく、人数に応じて**計算で配置**するようにします。

##### ⑤ 抽出したルールを生成コードに置き換える
読み取った構造を、`merge()`（結合）・`fill_yellow()`（黄色塗り）・
`set_border()`（罫線）といった部品関数で再現します。

> **ポイント**：完成図を「絵」として真似るのではなく、
> *行の高さ・結合・罫線・塗り* という**数値ルールに分解**することで、
> どんな人数・パターンにも対応できる柔軟なツールになります。
""")
