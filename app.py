...
with st.expander('📖 このツールの開発について'):
    st.markdown("""
本ツールは、以下の法定相続情報一覧図の書式（記載例）を学習して開発しました。

- 配偶者・子（１人～４人まで対応）である場合
- 子（１人～４人まで対応）である場合
- 配偶者・親２名（父及び母）である場合
- 配偶者・親１名（父又は母）である場合
- 配偶者・兄弟姉妹（１人～３人まで対応）である場合
- 父母の一方のみを同じくする兄弟姉妹がいる場合
- 代襲相続が生じている場合

これらの書式を参考に、相続関係に応じたExcelひな形を自動生成しています。
""")

with st.expander('🛠 技術スタック'):
    st.markdown("""
本ツールは、以下の技術を用いて開発しています。
...
| 画面（UI） | Streamlit |
| Excel生成 | openpyxl |
| 相続関係プレビュー | SVG（プログラムによる図形描画） |
| バージョン管理 | Git / GitHub |
| 公開環境 | Streamlit Community Cloud |
""")

with st.expander('💬 開発時の主な指示（プロンプト）'):
    st.markdown("""
本ツールは、AIとの対話を重ねながら開発しました。
開発の過程で出した主な指示（プロンプト）の一部をご紹介します。

- 相続関係の入力に応じて、最適な書式を自動で判定・生成できるようにしたい
- 入力内容に応じて、相続関係図をその場でプレビュー表示したい
- 選択ボタンやアニメーションで、直感的に操作できるようにしたい
- 入力をすべてクリアできるリセット機能をつけたい
- 代襲相続の場合に、孫（代襲者）の人数を入力して書式に反映したい
- 続柄を結ぶ関係線を自動で描画したい
- 氏名・住所などの個人情報は、出力後に手入力するよう案内したい

これらの指示をもとに、機能の追加や改善を繰り返して現在の形になりました。
""")

with st.expander('🔍 参考書式の解析メモ（開発の記録）'):
    st.markdown("""
同じようなExcel自動生成ツールを作るときの参考になればと思い、
本ツールで**参考書式（Excel）をどう読み解いたか**の記録を残しておきます。
あくまで一つのやり方として、ご覧いただければ幸いです。

##### ① 書式を構造データとして読み取った
参考のExcelを `openpyxl` で開き、見た目を構成する要素を
数値として取り出しました。

| 取り出した要素 | 内容 |
|---|---|
| 行の高さ | 各行の高さ（被相続人欄は高め、補足欄は低め など） |
| セルの結合 | どの範囲が結合され、何の項目か（「最後の住所」等） |
| 罫線 | どのセルの上下左右に線があるか（続柄を結ぶ関係線など） |
| 塗りつぶし | 入力欄（黄色）の位置 |
...
wb = openpyxl.load_workbook("参考書式.xlsx")
ws = wb.active

# セルの結合範囲とその中身を読み取る
for rng in ws.merged_cells.ranges:
    cell = ws.cell(rng.min_row, rng.min_col)
    print(rng, cell.value)

# 罫線のある位置を読み取る
for row in ws.iter_rows():
    for c in row:
        b = c.border
        if b.top.style or b.left.style:
            print(c.coordinate, b.top.style, b.left.style)
```

##### ③ 「列のゾーン」に分けて考えた
横方向を3つの区画に分けて眺めたところ、配置の規則性がつかめました。

- 左ゾーン：相続人（子など）の氏名・住所
- 中央ゾーン：続柄ラベルと、関係を結ぶ縦・横の線
- 右ゾーン：配偶者や孫の氏名

##### ④ 人数に応じて座標を動かした
「1人増えるごとに6行ずつ下にずれる」といった規則に気づき、
固定の書式ではなく、人数に応じて計算で配置する形にしました。

##### ⑤ 読み取った構造を生成コードに置き換えた
抽出した内容を、`merge()`（結合）・`fill_yellow()`（黄色塗り）・
`set_border()`（罫線）といった部品関数で組み直しています。

> 完成図を「絵」として真似るより、*行の高さ・結合・罫線・塗り* という
> 数値ルールに分解してみたところ、いろいろな人数やパターンにも
> 対応しやすくなりました。何かの参考になれば嬉しいです。
""")
