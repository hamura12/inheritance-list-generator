...
def _base_borders(ws):
    set_border(ws,9,17,right=6)   # Q9 right=6
    set_border(ws,10,18,left=6)   # R10 left=6
    # K列縦線（被相続人→兄弟姉妹接続）
...
def _sN(ws, n):
    """兄弟姉妹N人（N≥4）拡張シート"""
    # 行高さ：n=3ベース（sibling 3まで＋gap38）＋sibling 4以降ブロック＋作成者
    ...
    _base_borders(ws)
    merge(ws,'D22','E22','（父）',alignment=al(2))
    merge(ws,'D27','E27','（母）',alignment=al(2))

    # 兄弟姉妹2のK列罫線（静的）
    set_border(ws,25,11,top=1,left=1)
    for c in range(12,16): set_border(ws,25,c,top=1)
    set_border(ws,26,5,left=6); set_border(ws,26,11,left=1)
...
def _build_sheet(wb, n, appl_start='X24', appl_end='AA25'):
    ws = wb.create_sheet(f'配偶者・兄弟姉妹{n}人')
    set_col_widths(ws)
